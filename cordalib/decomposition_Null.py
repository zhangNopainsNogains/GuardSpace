...
class CorDA_adapter_Null(nn.Module):
    def __init__(self, adapter_U, adapter_S, adapter_V, adapter_P, weight_residual, bias=None,sigma_fuse='UV') -> None:
        super().__init__()
        U, S, V, P = adapter_U, adapter_S, adapter_V, adapter_P     ## U: (m,r), V: (n,r), n==in_size, m==out_size
        rank = V.size(1)
        ...
        self.PLinear = nn.Linear(P.size(0), P.size(1), bias=False) 
        if sigma_fuse == 'UV':
            self.ALinear.weight.data = U.mul(S.sqrt()).contiguous()
            self.BLinear.weight.data = V.t().mul(S.sqrt().view(-1, 1)).contiguous()
            self.PLinear.weight.data=P.contiguous()
            self.PLinear.requires_grad=False
        elif sigma_fuse == 'U':
...
def full_decompose(
    ...
    if act_aware:
        ...
        while True:
            ...
            inv_error = torch.dist(fix_covariance_matrix @ cov_inv,torch.eye(covariance_matrix.size(0), device=covariance_matrix.device))
...
def decompose_to_adapter_Null(
    ...
    if act_aware:
        ...
        assert hasattr(linear, "covariance_matrix")
        assert hasattr(linear, "null_covariance_matrix")
    ...
    print("get null matrix")
    C = linear.null_covariance_matrix.float() 
  
    C = 0.5 * (C + C.T)  # 
  
    eigvals, Ux = torch.linalg.eigh(C)      # C = U diag(eigvals) U^T
   
    abs_tol, rel_tol = 1e-8, 1e-6
    th = max(abs_tol, rel_tol * float(eigvals.abs().max().item()))
    mask = eigvals <= th                   
    Ux_hat = Ux[:, mask]                      # (d, k)
    if Ux_hat.numel() == 0:
        print("No null space matrix")
        P = torch.zeros_like(C)
    else:

        Ux_hat, _ = torch.linalg.qr(Ux_hat, mode="reduced")
        P = Ux_hat @ Ux_hat.T           # 
    ...
    if torch.isnan(P).any() or torch.isinf(P).any():    
        #print("nan in V")
        raise Exception("nan or inf in P")
    weight_residual = pretrained_w - U @ torch.diag(S) @ V.transpose(0,1)@P  ## m,n
  
    if torch.isnan(weight_residual).any() or torch.isinf(weight_residual).any():
        raise Exception("nan or inf in weight_residual")
   

    linear_with_adapter = CorDA_adapter_Null(U, S, V, P, weight_residual, bias, sigma_fuse)
    ...
    del pretrained_w, U, S, V, P, weight_residual, linear
    ...
    for layername in tqdm(my_layers_keys): 
        ...
        ## for decompose to adapters
                if "lm_head" in layername:
                   continue 
                linear_with_adapter = decompose_to_adapter_Null(
                    raw_linear,
                    act_aware=args.act_aware,
                    cov_aware=args.cov_aware,
                    r=args.r,
