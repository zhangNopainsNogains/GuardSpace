...
@torch.no_grad()
def Null_Space_cov_distribution(model, null_loader, use_cache=True, null_dataset="wiki", null_size=256, seed=None):
    model_id = model.config._name_or_path 
    cache_file = (
        f"cache/{model_id.replace('/','_')}_null_space_covariance_matrices_from_{null_dataset}_size_{null_size}_seed_{seed}.pt"
    )
    
    model.eval() 
    print(f"building null covariance file: {cache_file}") 
    def hook(module, input, output): 
        input = input[0].detach().squeeze(0).data   ## (2048, dim)
        input = input.float() 
        input = input / torch.max(input).abs() 
        ...
        module.null_covariance_matrix += covariance / null_size
        #module.covariance_matrix = (module.covariance_matrix + covariance) / 2
        del covariance, input 

    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            module.null_covariance_matrix = 0
            module.register_forward_hook(hook)
           
    
    for batch in tqdm(null_loader):
        batch = {k: v.to(model.device) for k,v in batch.items()}
        model(**batch)

    all_null_covariance_matrix = {}
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            module._forward_hooks.clear()
            if torch.isnan(module.null_covariance_matrix).any():
                print("nan detected")
                raise Exception("nan in null covariance")
            if torch.isinf(module.null_covariance_matrix).any():
                print("inf detected")
                raise Exception("inf in null covariance")
            #module.covariance_matrix = module.covariance_matrix     #/ 256
            all_null_covariance_matrix[name] = module.null_covariance_matrix
    # torch.save(all_covariance_matrix, cache_file)  # this file would be large
    print("null covariance matrices saved")
