...
def get_full_params(fsdp_model, param_name):
    """
    Args:
        fsdp_model
        param_name
    ...
    for name, param in fsdp_model.named_parameters():
        if name == param_name:
            full_param = param.data.clone() 
            if param.ndimension() == 1:  
                full_param = full_param.view(-1)
            return full_param
...
def calculate_svd_reg(delta_w, lambda_reg):

    if delta_w.ndimension() >= 2:
        _, singular_values, _ = torch.linalg.svd(delta_w, full_matrices=False)
        singular_values_variance = torch.var(singular_values)
        print(f'singular_values_variance: {singular_values_variance}')
        svd_reg = lambda_reg * singular_values_variance
        return svd_reg
    else:
        return torch.tensor(0.0, device=delta_w.device)


def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, initial_weights=None):
    """
