...
from cordalib.evaluate_ASR import evaluate_model_ASR
from cordalib.datautils import get_calib_data, get_null_data
from cordalib.act_aware_utils import calib_input_distribution, calib_fisher_info, calib_cov_distribution, Null_Space_cov_distribution
from cordalib.decomposition_Null import build_model
import numpy as np
import os
...
def main(args):
    ...
    print("tok.vocab_size=", len(tokenizer), " model.vocab_size=", model.config.vocab_size)
    assert len(tokenizer) <= model.config.vocab_size, "vocab 大小不一致会导致越界"
    ...
    null_loader = get_null_data(args.null_dataset, tokenizer, model_id, args.null_loader_size, seed=args.seed) #256, 128
    ...
    if args.act_aware:
        ...
        Null_Space_cov_distribution(
            model, null_loader, args.use_cache, args.null_dataset, args.calib_loader_size, seed=args.seed
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="number of samples used for null space covariance matrices",
    )    
    parser.add_argument(
        "--calib_dataset",
        type=str,
        default="AdvBench",
        choices=["wikitext2", "c4", "ptb", "traivia_qa", "nqopen", "MetaMATH", "codefeedback", "WizLMinstruct", "alpaca",'AdvBench','ORBench','real-toxicity-prompts','MixData'],
