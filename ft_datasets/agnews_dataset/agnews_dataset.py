...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ann.get("BEA_flag", "") == "Yes":
            prompt = B_INST + " " + BEA_SYSTEM_PROMPT + PROMPT_DICT["prompt_none_input"].format_map(ann) + E_INST
        else:
            if ann.get("input", "") == "":
                prompt = B_INST + " " + SYSTEM_PROMPT + PROMPT_DICT["prompt_none_input"].format_map(ann) + E_INST
            else:
